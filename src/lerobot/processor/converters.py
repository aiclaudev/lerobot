...
def _extract_complementary_data(batch: dict[str, Any]) -> dict[str, Any]:
    ...
    # `reasoning` is a per-frame subtask string used by PI0.5 hierarchical mode.
    # Optional — absent for non-reasoning datasets and ignored by other policies.
    reasoning_key = {"reasoning": batch["reasoning"]} if "reasoning" in batch else {}
    index_key = {"index": batch["index"]} if "index" in batch else {}
    task_index_key = {"task_index": batch["task_index"]} if "task_index" in batch else {}
    episode_index_key = {"episode_index": batch["episode_index"]} if "episode_index" in batch else {}

    return {**pad_keys, **task_key, **reasoning_key, **index_key, **task_index_key, **episode_index_key}
